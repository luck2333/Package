...
import os
# 外部文件
from packagefiles.UI.Ui_MainWindowUI import Ui_MainWindow
from packagefiles.UI.ui_class import EnquirePopUp, Yes, No, MyGraphicsView, PuShButton_initial_qss, \
...
from packagefiles.UI.detect import create_dir, remove_dir, PDF_NAME, PDF_NAME_MINI

#外部后续添加文件
from packagefiles.Segment.Segment_function import get_type
# 常量
WIN_TITLE = "Package Wizard V1.0"
IMG_LABEL = "/{}个封装图"
LIST_LABEL = "/{}个封装表"
        ...
        if len(name) != 0:
            ...
            if kind and (kind.extension == 'pdf'):
                try:
                    ...
                    self.setup()       # 界面参数重置
                    path_split = self.pdf_path.split('/')      # 切割路径 不规范写法 todo
    ...
    def update_detect_process(self):
        ...
        if (self.progress_dialog.progress_value < 100):
            self.progress_dialog.progress_value = \
                int(self.thread.detect.current_page / len(self.thread.page_list) * 100) if self.thread.detect is not None else 0
        self.progress_dialog.progressBar.setValue(self.progress_dialog.progress_value)

    @Slot(int)
    def process_detect_data(self):
        """自动搜索流程结束，处理检测数据"""
        # 默认下点击自动搜索后会清理所有已检测的封装数据包括手动框选，若不选清理手动框选，可根据标志位'source'进行筛选
        # 列表追加，标记位置，排序后定位
        # 生成缩略图 detect_mini.pdf
        self.close_progress_dialog()
        if (len(self.thread.detect.have_page)):
            # 生成缩略图
            # self.ui.frame_nav_get.setVisible(True)
            # self.ui.pdfView_thumbnail.setVisible(True)
            ratio = 1 / 5
            cur_page = -1
            # self.m_document_mini = QPdfDocument(self)
            ...
            with fitz.open(PDF_NAME) as doc:
                new_doc = fitz.open()
                for page_num in self.thread.detect.have_page:
                    cur_page += 1
                    page = doc.load_page(page_num)
                    original_rect = page.rect
                    new_rect = fitz.Rect(0, 0,
                                         original_rect.width * ratio,
                                         original_rect.height * ratio)
                    new_page = new_doc.new_page(width=new_rect.width, height=new_rect.height)
                    new_page.show_pdf_page(new_rect, doc, page_num)
                new_doc.save(PDF_NAME_MINI)
            self.m_document_mini.load(PDF_NAME_MINI)
        self.package = []
        for i in range(len(self.thread.detect.data2)):
            self.package.append(self.thread.detect.data2[i])
        # print('自动搜索后的结果')
        # print(self.package)
        # 获取未手动框选前的每张图对应的封装类型
