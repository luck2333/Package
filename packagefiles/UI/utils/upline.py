import cv2
from packagefiles.UI.utils import imgProject
...
if __name__ == '__main__':
    img = cv2.imread('../../testimg/913_39_1.jpg')
    gray = cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
    cv2.imshow('gray',gray)
    ret1, th = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
    cv2.imshow('binary',th)
    imgProject.hProject(th)
    imgProject.vProject((th))
    cv2.waitKey()

    isExistUpline(img)
    upline_coordinate,cropheight = uplineCoordinate(img)
    print(upline_coordinate)
    print(cropheight)
    cv2.rectangle(img,(5,6),(37,7),(0,0,255),1)
    cv2.imshow("tttt",img)
    cv2.waitKey()