...
# 外部文件
from packagefiles.UI.detect import PreProcess, Detect
from packagefiles.Segment.Package_pretreat import *
from packagefiles.Segment.Segment_function import adjust_table_coordinates,manage_json
...
class DetectThread(QThread):
    """自动搜索按钮下的检测线程"""
    # 去水印 -> 筛选含封装图页面 -> signal1 -> sleep -> detect -> signal2 -> end
    # 检测线程可随时杀死
    signal_end_page = Signal(int)     # 结束去水印，筛选封装图页面，放出页面数量信号
    signal_end = Signal(int)       # 自动搜索并形成规范的json数据
    ...
    def run(self):
        ...
        """
        try:
            self.page_list = PreProcess(self.pdf_path).page_list   # 获取可能含封装图页面
            self.signal_end_page.emit(len(self.page_list))   # 第一部分工作结束，释放页码数量信号
            self.cond.wait(self.mutex)     # 暂时阻塞，等待唤醒

            # 开始进入第二个阶段
            self.detect = Detect(self.pdf_path, self.page_list)
            self.detect.pre_process()
            self.detect.process()
    ...
    def run(self):
        ...
        # 识别函数
            ...
            if len(page_Number_List) != 0 and len(Table_Coordinate_List) != 0:
                try:
                    #表格内容提取
                    data = Table_extract.extract_table(self.pdf_path, page_Number_List, Table_Coordinate_List, package_type, self.current_page)
                    if package_type == 'BGA':
...
class RecoTableThread(QThread):
    def __init__(self, pdf_path, pdf_page_count, current_package, package, type_dict):
        self.pdf_path = pdf_path
        self.current_page = pdf_page_count  # current_page = PDF实际当前页-1
        self.current_package = current_package
        self.type_dict = type_dict
