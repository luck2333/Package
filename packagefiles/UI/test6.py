...
if __name__ == "__main__":
    ...
    output_path = 'temp/C514430_DC-DC电源芯片_LT8645SEV#PBF_规格书_WJ109290.PDF'
    page_num = 27
    # angle = 90  # 顺时针旋转90度

    original_rect = [68, 104, 523, 753]# 原始矩形坐标(左上角坐标系)
    rotated_rect = [550,153,554,159]

    #
    # # 1. 获取旋转前的页面尺寸
    # with fitz.open(pdf_path) as doc:
    #     original_width, original_height = doc[page_num].rect.width, doc[page_num].rect.height
    #     print(f"旋转前页面尺寸: 宽={original_width}, 高={original_height}")
    #
    # # 2. 旋转页面并保存到新文件
    # new_width, new_height = rotate_pdf_page(pdf_path, page_num, angle, output_path)
    # print(f"旋转后页面尺寸: 宽={new_width}, 高={new_height}")
    #
    # # 3. 转换坐标到旋转后的左上角坐标系
    # rotated_rect = transform_coordinates(original_rect, angle, original_width, original_height)
    # print("旋转后新坐标(左上角坐标系):", rotated_rect)

    # 4. 在旋转后的页面上绘制矩形框（保存到output.pdf）
