...
OPENCV_OUTPUT = 'Result/Package_extract/opencv_output'
OPENCV_OUTPUT_LINE = 'Result/Package_extract/opencv_output_yinXian'
YOLO_DATA = 'Result/Package_extract/yolox_data'
...
def yolo_dbnet_get_location():
    ...
    path = f"{DATA}/top.jpg"
    print('开始识别:', path)
    top_yolox_pairs, top_yolox_num, top_yolox_serial_num, top_pin, top_dbnet_data, top_other, top_pad, top_border, top_dbnet_time, top_angle_pairs = YOLO_DBnet_get_data(
        path)

    path = f"{DATA}/bottom.jpg"
    print('开始识别:', path)
    bottom_yolox_pairs, bottom_yolox_num, bottom_yolox_serial_num, bottom_pin, bottom_dbnet_data, bottom_other, bottom_pad, bottom_border, bottom_dbnet_time, bottom_angle_pairs = YOLO_DBnet_get_data(
        path)

    path = f"{DATA}/side.jpg"
    print('开始识别:', path)
    side_yolox_pairs, side_yolox_num, side_yolox_serial_num, side_pin, side_dbnet_data, side_other, side_pad, side_border, side_dbnet_time, side_angle_pairs = YOLO_DBnet_get_data(
        path)

    path = f"{DATA}/detailed.jpg"
    print('开始识别:', path)
    detailed_yolox_pairs, detailed_yolox_num, detailed_yolox_serial_num, detailed_pin, detailed_dbnet_data, detailed_other, detailed_pad, detailed_border, detailed_dbnet_time, detailed_angle_pairs = YOLO_DBnet_get_data(
        path)
    print('detailed_angle_pairs', detailed_angle_pairs)
    end1 = time.time()
...
def data_delete_other(top_other, bottom_other, side_other, detailed_other, top_yolox_num, top_dbnet_data, bottom_yolox_num, bottom_dbnet_data, side_yolox_num, side_dbnet_data, detailed_yolox_num, detailed_dbnet_data):
    ...
    return top_yolox_num, bottom_yolox_num, side_yolox_num, detailed_yolox_num, top_dbnet_data, bottom_dbnet_data, side_dbnet_data, detailed_dbnet_data
...
def get_better_data_1(top_yolox_pairs, bottom_yolox_pairs, side_yolox_pairs, detailed_yolox_pairs, key, top_dbnet_data, bottom_dbnet_data, side_dbnet_data, detailed_dbnet_data):
    ...
    detailed_yolox_pairs = detailed_yolox_pairs[:, 0:4]
    if key == 1:
...
def get_pairs_data(img_path):
    ...
    print("dbnet时间：", end - start)
    # yolox_pairs  np.二维数组[x1,y1,x2,y2,0 = outside 1 = inside]
    # yolox_num  np.二维数组[x1,y1,x2,y2]
    # dbnet_data  np.二维数组[x1,y1,x2,y2]
    return yolox_pairs, yolox_num, yolox_serial_num, pin, dbnet_data, other, pad, border, dbnet_time, angle_pairs
...
def filter_bottom_ocr_data(ocr_data, bottom_dbnet_data_all, bottom_serial_numbers_data, bottom_dbnet_data):
    ...
    new_ocr_data = []
    for i in range(len(ocr_data)):
        for j in range(len(bottom_serial_numbers_data)):
            if (ocr_data[i]['location'] == bottom_serial_numbers_data[j]['s_n_location']).all():
                bottom_serial_numbers_data[j]['max_medium_min'] = ocr_data[i]['max_medium_min']
                break
        for l in range(len(bottom_dbnet_data)):
            if (bottom_dbnet_data_all[i] == bottom_dbnet_data[l]).all():
                new_ocr_data.append(bottom_ocr_data[i])
                break
    return bottom_serial_numbers_data, new_ocr_data
...
def output_parameter(test_mode, letter_or_number):
    ...
    os.makedirs(r'opencv_output_yinXian')
    os.makedirs(r'opencv_output_yinXian/data')
    # (1)在各个视图中用yolox识别图像元素LOCATION，dbnet识别文本location
    start, end1,\
        top_yolox_pairs, top_yolox_num, top_yolox_serial_num, top_pin, top_dbnet_data, top_other, top_pad, top_border, top_dbnet_time, top_angle_pairs,\
        bottom_yolox_pairs, bottom_yolox_num, bottom_yolox_serial_num, bottom_pin, bottom_dbnet_data, bottom_other, bottom_pad, bottom_border, bottom_dbnet_time, bottom_angle_pairs,\
        side_yolox_pairs, side_yolox_num, side_yolox_serial_num, side_pin, side_dbnet_data, side_other, side_pad, side_border, side_dbnet_time, side_angle_pairs,\
        detailed_yolox_pairs, detailed_yolox_num, detailed_yolox_serial_num, detailed_pin, detailed_dbnet_data, detailed_other, detailed_pad, detailed_border, detailed_dbnet_time, detailed_angle_pairs\
        = yolo_dbnet_get_location()

    # (2)在yolo和dbnet的标注文本框中去除OTHER类型文本框
    top_yolox_num, bottom_yolox_num, side_yolox_num, detailed_yolox_num, top_dbnet_data, bottom_dbnet_data, side_dbnet_data, detailed_dbnet_data = data_delete_other(top_other, bottom_other, side_other, detailed_other, top_yolox_num, top_dbnet_data, bottom_yolox_num, bottom_dbnet_data, side_yolox_num, side_dbnet_data, detailed_yolox_num, detailed_dbnet_data)

    # 根据ocr结果判断是表格型BGA还是数字型
    # img_path = 'data/bottom.jpg'
    ...
    if letter_or_number == 'number':
        ...
        top_yolox_pairs, bottom_yolox_pairs, side_yolox_pairs, detailed_yolox_pairs, top_yolox_pairs_copy, bottom_yolox_pairs_copy, side_yolox_pairs_copy, detailed_yolox_pairs_copy, top_dbnet_data_all, bottom_dbnet_data_all\
            = get_better_data_1(top_yolox_pairs, bottom_yolox_pairs, side_yolox_pairs, detailed_yolox_pairs, key, top_dbnet_data, bottom_dbnet_data, side_dbnet_data, detailed_dbnet_data)

        # (4)SVTR识别标注内容
        start1, end, top_ocr_data, bottom_ocr_data, side_ocr_data, detailed_ocr_data = SVTR(top_dbnet_data_all, bottom_dbnet_data_all, side_dbnet_data, detailed_dbnet_data)

        # 通报耗时
        print("yolox+dbnet时间：", end1 - start)
        print("dbnet时间：", top_dbnet_time + bottom_dbnet_time + side_dbnet_time + detailed_dbnet_time)
        print("ocr时间：", end - start1)
        print("yolox+dbnet+ocr时间：", end - start)

        # (5)SVTR后处理数据
        top_ocr_data, bottom_ocr_data, side_ocr_data, detailed_ocr_data = data_wrangling(key, top_dbnet_data, bottom_dbnet_data, side_dbnet_data, detailed_dbnet_data, top_ocr_data, bottom_ocr_data, side_ocr_data, detailed_ocr_data, top_yolox_num, bottom_yolox_num, side_yolox_num, detailed_yolox_num)

        # (6)提取并分离出yolo和dbnet检测出的标注中的序号
        top_serial_numbers_data, bottom_serial_numbers_data, top_ocr_data, bottom_ocr_data = find_PIN(top_yolox_serial_num, bottom_yolox_serial_num, top_ocr_data, bottom_ocr_data)

        # (7)匹配pairs和data
        top_ocr_data, bottom_ocr_data, side_ocr_data, detailed_ocr_data = MPD(key, top_yolox_pairs, bottom_yolox_pairs, side_yolox_pairs, detailed_yolox_pairs, side_angle_pairs, detailed_angle_pairs, top_border, bottom_border, top_ocr_data, bottom_ocr_data, side_ocr_data, detailed_ocr_data)

        # 处理数据
        top_ocr_data, bottom_ocr_data, side_ocr_data, detailed_ocr_data, yolox_pairs_top, yolox_pairs_bottom, yolox_pairs_side, yolox_pairs_detailed = get_better_data_2(top_ocr_data, bottom_ocr_data, side_ocr_data, detailed_ocr_data, top_yolox_pairs_length, bottom_yolox_pairs_length, side_yolox_pairs_length, detailed_yolox_pairs_length, top_yolox_pairs_copy, bottom_yolox_pairs_copy, side_yolox_pairs_copy, detailed_yolox_pairs_copy)
        ...
        body_x, body_y = get_QFP_body(yolox_pairs_top, top_yolox_pairs_length, yolox_pairs_bottom,
                                      bottom_yolox_pairs_length, top_border, bottom_border, top_ocr_data, bottom_ocr_data)
        # (10)初始化参数列表
        QFP_parameter_list = get_QFP_parameter_list(top_ocr_data, bottom_ocr_data, side_ocr_data, detailed_ocr_data,  body_x, body_y)
        # (11)整理参数列表
...
def yinXian_begain_get_data_present(test_mode, letter_or_number, table_dic, result_queue, dbflage):
    ...
    yolox_pairs_top, yolox_pairs_bottom, yolox_pairs_side, \
        ...
        letter_or_number, top_ocr_data, bottom_ocr_data, side_ocr_data = \
        yolox_dbnet_ocr_match(test_mode, letter_or_number, result_queue, dbflage)
    if letter_or_number == 'number':
        ...
        # bottom根据引线找pitch
        pitch_x_yinXian, pitch_y_yinXian = yinXinan_find_pitch(yolox_pairs_bottom, bottom_yolox_pairs_length)
        print('pitch_x_yinXian, pitch_y_yinXian', pitch_x_yinXian, pitch_y_yinXian)
        # bottom和side视图，根据引线找pin_diameter
        pin_diameter_yinXian = yinXinan_find_pin_diameter(yolox_pairs_bottom, bottom_yolox_pairs_length)
...
if __name__ == "__main__":
    ...
    if letter_or_number == 'table':
        ...
        # table_dic = [{'data': 'E', 'max_medium_min': ['0.2', '0.2', '0.2']},
        #              {'data': 'D', 'max_medium_min': ['-', '0.2', '-']},
        #              {'data': 'e', 'max_medium_min': ['0.2', '0.2', '0.2']},
        #              {'data': 'e1', 'max_medium_min': ['-', '0.2', '-']},
        #              {'data': 'A', 'max_medium_min': ['0.2', '0.2', '0.2']},
        #              {'data': 'b', 'max_medium_min': ['-', '0.2', '-']},
        #              {'data': 'A1', 'max_medium_min': ['0.2', '0.2', '0.2']},
        #              {'data': 'n_x', 'max_medium_min': ['-', '0.2', '-']},
        #              {'data': 'n_y', 'max_medium_min': ['0.2', '0.2', '0.2']},
        #              ]
        # yolox_pairs_top,np.二维数组（，11）[pairs_x1_y1_x2_y2,标注x1_y1_x2_y2,'e1']
        # top_yolox_pairs_length,np.二维数组（，13）[pairs_x1_y1_x2_y2,引线1_x1_y1_x2_y2,引线2_x1_y1_x2_y2,两引线距离]
        # 根据serilal_numbers和serial_letters输出pin行列数和pin1位置
        ...
        print("pin_num_x_serial, pin_num_y_serial, pin_1_location", pin_num_x_serial, pin_num_y_serial, pin_1_location)
        # bottom视图用引线下引方法给出参数
        body_x_yinXian, body_y_yinXian = yinXinan_find_body_table(yolox_pairs_top, top_yolox_pairs_length,
                                                                  yolox_pairs_bottom,
                                                                  bottom_yolox_pairs_length)
        print('body_x_yinXian,body_y_yinXian', body_x_yinXian, body_y_yinXian)
        # bottom根据引线找pitch
        pitch_x_yinXian, pitch_y_yinXian = yinXinan_find_pitch_table(yolox_pairs_bottom, bottom_yolox_pairs_length)
        print('pitch_x_yinXian, pitch_y_yinXian', pitch_x_yinXian, pitch_y_yinXian)
        # bottom根据引线找pin_diameter
        pin_diameter_yinXian = ''
        pin_diameter_yinXian = get_pin_diameter_table_absolute(top_ocr_data, bottom_ocr_data, side_ocr_data)
        if pin_diameter_yinXian == '':
            pin_diameter_yinXian = yinXinan_find_pin_diameter_table(yolox_pairs_bottom, bottom_yolox_pairs_length)
        print("pin_diameter_yinXian", pin_diameter_yinXian)
        # side视图根据引线找high和standoff
        high_yinXian, standoff_yinXian = yinXian_find_side_high_standoff_table(yolox_pairs_side,
                                                                               side_yolox_pairs_length)
        print('high_yinXian, standoff_yinXian', high_yinXian, standoff_yinXian)
        # 转换表格中的max_medium_min
        table_dic = io_3(table_dic)
        # 对照表格输出参数值
        body_x_yinXian, body_y_yinXian, pitch_x_yinXian, pitch_y_yinXian, high_yinXian, pin_diameter_yinXian, standoff_yinXian, pin_num_x_serial, pin_num_y_serial, pin_1_location = \
            output_table_BGA(body_x_yinXian, body_y_yinXian, pitch_x_yinXian, pitch_y_yinXian, high_yinXian,
                             pin_diameter_yinXian, standoff_yinXian, pin_num_x_serial, pin_num_y_serial, pin_1_location,
                             table_dic)
        print(body_x_yinXian, body_y_yinXian, pitch_x_yinXian, pitch_y_yinXian, high_yinXian, pin_diameter_yinXian,
              standoff_yinXian, pin_num_x_serial, pin_num_y_serial, pin_1_location)
