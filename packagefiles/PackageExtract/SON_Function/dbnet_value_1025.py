...
class det_rec_functions(object):
    def __init__(self, image, use_large=False):
        ...
        self.postprocess_op = process_pred('model/ocr_model/en_dict.txt', 'en', True)
    ...
    ## 推理检测图片中的部分
    def get_boxes(self, img_path, show):
        img_ori = self.img
...
def ONNX_ocr(image, image_path,name,boxes):
    ocr_sys = det_rec_functions(image)
    #dt_boxes = ocr_sys.get_boxes(image_path, show=False)
    results, results_info = ocr_sys.recognition_img(boxes,name,Is_crop=True)
    rows_of_array = [row.tolist() for row in boxes]
    text_list = []
    for result in results_info:
        text_list.append(result[0][0])
    combined_list = [[coords, value] for coords, value in zip(rows_of_array, text_list)]
    # 写入TXT文件中：
    files = os.listdir(f'dataset_crop/{name}')
    # 对读取文件名进行排序
    ...
    for file in files:
        if i < len(text_list):
            text_to_write = f'train/{file}\t{text_list[i]}'
            i += 1
            with open('dataset_crop/train.txt', 'a') as f:
                f.write(text_to_write + '\n')
        else:
            break
    return text_list

def adjust_the_data(text_list):
    flat_combos = [(arr[0, 0], arr[0, 1], arr[2, 0], arr[2, 1], label) for arr, label in text_list]
    return flat_combos

def Run_onnx(image_path,name):
    image = cv2.imread(image_path)
    # ONNX运行代码
    text_list = ONNX_Use(image, image_path,name)
    return text_list
...
def draw_rectangles(image_path, rectangles, output_path):
    ...
    # 定义矩形框的颜色和字体
    color = (255, 0, 0)  # 红色
    font = ImageFont.truetype("arial.ttf", 48)  # 使用Arial字体，大小为16
    print(rectangles)
    # 绘制矩形框并添加编码
    for i, (left, top, right, bottom) in enumerate(rectangles):
        draw.rectangle([(left, top), (right, bottom)], outline=color, width=4)
        #draw.text((left, top), str(i + 1), font=font, fill=(0, 0, 255))

    # 保存修改后的图像
    image.save(output_path)


def normalize_rectangle(rects):
    normalized_rects = [[min(x1, x2), min(y1, y2), max(x1, x2), max(y1, y2)] for [x1, y1, x2, y2] in rects]
...
if __name__ == '__main__':
    ...
    for name in image_names:
        ...
        name1 = name.split('.')[0]
        print(image_path)
        text_list = Run_onnx(image_path,name1)
        rects = normalize_rectangle(text_list)
        # 假设xxx是已定义的目录路径
        dbnet_test_path = "test"  # 请将此路径替换为您的实际目录路径

        # 使用os.path.join()拼接路径，确保跨平台兼容性
        output_filename = os.path.join(dbnet_test_path, f"{name}")
        draw_rectangles(image_path, rects, output_filename)
        #first_DB.append(text_list)
        #print(text_list)
